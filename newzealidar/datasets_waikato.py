...
def get_extent_info(extent_file: Union[str, pathlib.Path]) -> gpd.GeoDataFrame:
    ...
    gdf = gdf.reset_index()
    gdf = gdf.rename(
        columns={"ID": "id", "METADATA": "meta_source", "VDATUM": "original_datum"}
    )
    return gdf
...
def store_dataset_to_db(
    ...
    tile_path_list = [
        str(pathlib.PurePosixPath(t / pathlib.Path(t.stem + "_TileIndex.zip")))
        for t in tile_dir
    ]
    dataset_dict["survey_end_date"] = [
    ...
    for index, row in gdf_to_db.iterrows():
        tile_path = [
            t for t in tile_path_list if "_".join(row["name"].split("_")[0:2]) in t
        ]
        ...
        gdf_to_db.loc[index, "extent_path"] = str(
            pathlib.PurePosixPath(
                dataset_dir / pathlib.Path("LiDAR_Regional_Extent.kml")
            )
        )
