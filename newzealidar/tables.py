...
from sqlalchemy import Column, Integer, Float, String, Interval, Date, DateTime, inspect
...
class TILE(Base):
    ...
    geometry = Column(
        Geometry("POLYGON", srid="2193"), comment="point cloud file extent"
    )
...
class CATCHMENT(Base):
    ...
    catch_id = Column(
        Integer, primary_key=True, comment="catchment index"
    )  # catchment region id
    area = Column(Float, comment="catchment area in square meters")
    geometry = Column(
        Geometry("Geometry", srid="2193"), comment="catchment area extent"
    )
    created_at = Column(DateTime)
...
class DEMATTR(Base):
    ...
    catch_id = Column(
        Integer, primary_key=True, comment="selected catchment index"
    )  # catchment region id
...
class RIVER(Base):
    __tablename__: str = "river"
    catch_id = Column(
        Integer, primary_key=True, comment="catchment index"
    )  # catchment region id
    rec_id = Column(Integer, comment="REC1 NZREACH")
    osm_id = Column(Integer, comment="OSM ID")
    rec_geometry = Column(
        Geometry("Geometry", srid="2193"), comment="REC1 NZREACH geometry"
    )
    osm_geometry = Column(Geometry("Geometry", srid="2193"), comment="OSM ID geometry")
    created_at = Column(DateTime)
    updated_at = Column(DateTime)
...
# record the process status for each catchment
class RUNTIME(Base):
...
def prepare_to_db(
    gdf_in: gpd.GeoDataFrame, check_empty: bool = True
) -> gpd.GeoDataFrame:
    """
...
def check_columns_up_to_3(
    table: Type[Ttable], column_1: str, column_2: str, column_3: str
) -> tuple:
...
def check_table_duplication(
    ...
    if c1 and not c2 and not c3:
        query = (
            f"SELECT COUNT(*) FROM {table} GROUP BY {column_1} HAVING COUNT(*) > 1 ; "
        )
    ...
    if count_all > 0:
        logger.warning(
            f"Table {table} duplication check: columns: 1 {column_1}, 2 {column_2}, 3 {column_3}, "
...
def deduplicate_table(
    ...
    logger.debug(
        f"Table {table} deduplicating: columns: 1 {column_1}, 2 {column_2}, 3 {column_3} with index {index}."
    )
    engine.execute(query)
...
def get_adjacent_catchment_by_id(
    engine: Engine,
    table: Type[Ttable],
    index: Union[int, str, list],
...
def get_catchment_by_geometry(
    ...
    :param relation: spatial relation, derived from PostGIS,
                     default 'ST_Contains', other options: 'ST_Intersects', 'ST_Within', etc.
    ...
    if desc is None or index is None:
        query = f"""SELECT * FROM {table}
                    WHERE {relation}({geom_col}, ST_SetSRID('{geom}'::geometry, 2193)) ;"""
    elif desc:
        query = f"""SELECT * FROM {table}
                    WHERE {relation}({geom_col}, ST_SetSRID('{geom}'::geometry, 2193)) AND catch_id <= {index} ;"""
    else:
        query = f"""SELECT * FROM {table}
                    WHERE {relation}({geom_col}, ST_SetSRID('{geom}'::geometry, 2193)) AND catch_id >= {index} ;"""
...
def read_postgres_table(
    engine: Engine, table: Union[str, Type[Ttable]], limit: int = 0
) -> pd.DataFrame:
...
def get_max_value(
    engine: Engine, table: Union[str, Type[Ttable]], column: str = "id"
) -> Union[int, float]:
...
def get_id_under_area(
    engine: Engine, table: Union[str, Type[Ttable]], area: Union[int, float]
) -> list:
...
def get_catchment_by_area_range(
    engine: Engine,
    table: Union[str, Type[Ttable]],
    upper_area: Union[int, float, None],
    lower_area: Union[int, float, None],
) -> gpd.GeoDataFrame:
    """
    Get catchment id list by area limit.
    ...
    if upper_area is not None and lower_area is None:
        ...
        raise ValueError(
            "Error: upper_area and lower_area cannot be None at the same time."
        )
    gdf = gpd.read_postgis(query, engine, geom_col="geometry")
...
def get_split_catchment_by_id(
    engine: Engine, index: Union[str, int, list], sub: bool = True
) -> list:
    """
    ...
    gdf = gpd.read_postgis(query, engine, geom_col="geometry")
    result = (
        sorted(gdf["catch_id"].to_list())
        if sub
        else sorted(list(set(gdf["super_id"].to_list())))
    )
    return result
