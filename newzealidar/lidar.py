...
def get_roi_from_file(
    file_path: Union[str, pathlib.Path], crs: str = "epsg:2193"
) -> gpd.GeoDataFrame:
...
def get_lidar_data(
    ...
    if gdf is not None and not gdf.empty:
        ...
        dataset is not None
    ):  # to process OpenTopography private datasets, if we use polygon search, geoapi will exit.
        dataset = [dataset] if not isinstance(dataset, list) else dataset
        logging.info(f"Start downloading dataset:\n{dataset}")
        for dataset_name in dataset:
            tile_file = pathlib.Path(data_path) / pathlib.Path(
                f"{dataset_name}" / pathlib.Path(f"{dataset_name}_TileIndex.zip")
            )
            if tile_file.exists():
                logger.info(
                    f"Tile index file {tile_file} already exists, delete it to download new files."
                )
                tile_file.unlink()
            lidar_fetcher = geoapis.lidar.OpenTopography(
                cache_path=data_path,
...
def gen_tile_data(gdf_in: gpd.GeoDataFrame, dataset: str) -> gpd.GeoDataFrame:
    ...
    if "2193" not in str(crs):
        logger.warning(
            f"Tile index file of {dataset} has crs {str(crs)}, converting to epsg:2193."
        )
...
def gen_lidar_data(
    ...
    if not (df["file_path"] == "").any():  # data from opentopography
        ...
        df["file_path"] = df["file_path"].apply(
            lambda x: x.replace(
                "https://opentopography.s3.sdsc.edu/pc-bulk/",
    ...
    df_downloaded = pd.DataFrame(
        {"file_name": file_name_list, "file_path": file_path_list}
    )
    ...
    if not (df["file_path"] == "").any():  # opentopograhy datasets
        ...
        df = df[["uuid", "file_name"]].merge(
            df_downloaded, on=["file_name"], how="right"
        )
    ...
    if len(nan_rows) > 0:
        logger.warning(
            f"{len(nan_rows)} .laz files are not found in the tile index file.\n"
...
def run(
    roi_id: Union[int, str, list] = None,
