...
split large catchments to small one, and save to local database.
"""
# import time
import gc
import logging
...
LOWER_AREA = CATCHMENT_RESOLUTION * CATCHMENT_RESOLUTION - EPS  # if area is smaller than this, it will be filtered out.
...
# mapping dictionary
GAP_TABLE_MAPPING = {tables.SDCP.__tablename__: tables.SDCG,
                     tables.CATCHMENT.__tablename__: tables.CATCHMENTG}

# now = time.strftime("%Y%m%d-%H%M%S")
# file = pathlib.Path(__file__).stem
# log_file = pathlib.Path(__file__).parent.parent / "logs" / f"{file}-{now}.log"
...
def fetch_data_from_mfe(layer: int,
                        crs: int = 2193,
...
def gen_source_catchment_table(engine: Engine, gpkg: bool = False) -> None:
    ...
    list_layers = [99776, 52363, 52364]  # sea draining catchments, order 5 catchments, order 4 catchments
    ...
    for layer, table, column in zip(list_layers, list_tables, list_columns):
        ...
        logger.info(f"Finish fetching {table.__tablename__} data from data.mfe.govt.nz.")
        if gpkg:
...
def check_duplicate_catchments(gds: Union[gpd.GeoSeries, pd.Series],
                               table: Type[tables.Ttable],
                               buffer: Union[int, float] = -10) -> list:
    """
    ...
    gdf = tables.get_within_catchment_by_geometry(engine, table, gds, buffer)
    engine.dispose()
    gc.collect()
    assert len(gdf) >= 1, f"Unexpected check result, it must contains itself: {gds['catch_id']} {table.__tablename__}"
    list_result = []
    if len(gdf) > 1:
...
# @utils.timeit
def deduplicate_single_table(source_table: Type[tables.Ttable],
    ...
    if parallel:  # SDC 0:08:49.020825, ORDER4 0:00:03.879000, ORDER5 0:00:38.870021
        ...
        with Pool(processes=multiprocessing.cpu_count()) as pool:
            list_result = pool.starmap(check_duplicate_catchments, zip(list_gds, repeat(source_table), repeat(buffer)))
            pool.close()
            pool.join()
    else:
        list_result = []
    ...
    if list_deduplicate:
        ...
        logger.info(f"Found {len(list_deduplicate)} duplicate catchments in table {source_table.__tablename__}.")
        logger.info(f"Deduplicate data and save data in table {processed_table.__tablename__}, "
                    f"duplicated catchments:\n{list_deduplicate}")
    else:
        gdf_to_db = gdf.copy()
        logger.info(f"No duplicate catchments found in table {source_table.__tablename__}.")
    ...
    engine = utils.get_database()
    gdf_to_db.to_postgis(processed_table.__tablename__, engine, index=True, if_exists='replace', chunksize=4096)
    engine.dispose()
    gc.collect()


# @utils.timeit  # 0:09:16.738249 for SDC, ORDER4, ORDER5 in total; 0:07:20.119389 for SDCP; 0:09:49.277458 for CATCHTMP
def deduplicate_table(source_table: Union[Type[tables.Ttable], list],
...
def extend_boundary(index: int, list_id: list, table: Type[tables.Ttable]) -> gpd.GeoDataFrame:
    """
    ...
    assert index in list_id and index in list_adj_id, f"Unexpected index: {index} not in {list_id} or {list_adj_id}"
    i = list_id.index(index)
    ignore = list_id[:i]
    list_adj_id = [i for i in list_adj_id if i not in ignore]
    if len(list_adj_id) < 2:
        if len(list_adj_id) == 0:
            logger.warning(f"Find empty geometry catchment, catch_id: {index}, adjacent_id: {list_adj_id}")
        if len(list_adj_id) == 1:
            assert list_adj_id[0] == index, f"unexpected adjacent_id: {list_adj_id} of catch_id: {index}."
            logger.debug(
    ...
    assert gdf_r.empty is False, f"unexpected empty rest geometry, catch_id: {index}, adjacent_id: {list_adj_id}"
    # original geometry - all adjacent catchments including the current index catchment
    geom_o = utils.filter_geometry(gdf['geometry'], resolution=CATCHMENT_RESOLUTION, polygon_threshold=LOWER_AREA)
    # rest geometry - all adjacent catchments excluding the current index catchment
    geom_r = utils.filter_geometry(gdf_r['geometry'], resolution=CATCHMENT_RESOLUTION, polygon_threshold=LOWER_AREA)
    ...
    geom_d = utils.filter_geometry(geom_d, resolution=CATCHMENT_RESOLUTION, polygon_threshold=LOWER_AREA)
    assert geom_d.area > EPS, f"unexpected empty difference geometry, catch_id: {index}, area: {geom_d.area}, " \
...
def extend_catchments(table: Type[tables.Ttable], parallel: bool = True, gpkg: bool = False) -> None:
    """
    ...
    engine = utils.get_database()
    gdf = tables.read_postgis_table(engine, table.__tablename__, sort_by='area', desc=True)
    assert gdf.empty is False, f"unexpected empty table {table.__tablename__}."
    list_area = gdf['area'].to_list()[:10]  # check top 10 largest catchments to save time
    assert all(list_area[i] >= list_area[i + 1] for i in range(len(list_area) - 1)), 'area should be sorted.'
    engine.dispose()
    gc.collect()
    ...
    if parallel:  # runtime: 0:24:10.686879.
        with Pool(processes=multiprocessing.cpu_count()) as pool:
            list_result = pool.starmap(extend_boundary, zip(list_id, repeat(list_id), repeat(table)))
            pool.close()
            pool.join()
    ...
    gdf_to_db.to_postgis(table.__tablename__, engine, index=True, if_exists="replace", chunksize=4096)
    engine.dispose()
    gc.collect()
...
def find_gaps(gdf_in: gpd.GeoDataFrame,
    ...
    # filtered geometry
    geom_f = utils.filter_geometry(gdf['geometry'],
    ...
    gdf_result = gdf_result.sort_values(by='area', ascending=False).reset_index(drop=True)
    ...
    if table is not None:
        ...
        engine = utils.get_database()
        tables.create_table(engine, table)
        gdf_to_db.to_postgis(table.__tablename__, engine, index=True, if_exists="replace", chunksize=1000)
        engine.dispose()
        gc.collect()
...
def merge_catchments(
    ...
    gdf = gdf_in.copy()
    assert gdf['catch_id'].is_unique, \
        f"catch_id is not unique in gdf: {sorted(gdf[gdf['catch_id'].duplicated()]['catch_id'].to_list())}"
    count_merge = 0
    count_skip = 0
    count_standalone = 0
    ...
    for i in range(len(gdf_merge)):
        if lower_area < gdf_merge.at[i, 'area'] < upper_area:  # to update existing catchments
            # find adjacent catchments
            gdf_adj = gdf_keep[
                gdf_keep['geometry'].intersects(
                    gdf_merge.at[i, 'geometry'].buffer(CATCHMENT_RESOLUTION / 2 - EPS, join_style='mitre')
                )
            ].copy()
            if gdf_adj.empty:
                logger.warning(f"Cannot find adjacent catchments for standalone catchment: "
                               f"{gdf_merge.at[i, 'catch_id']}, add it in catchment table.")
                gdf_row = gpd.GeoDataFrame(index=[0], crs='epsg:2193', geometry=[gdf_merge.at[i, 'geometry']])
            ...
            area = gdf_largest['geometry'].values[0].area  # use the calculated area instead
            # skip overlapped catchments
            if geometry.buffer(EPS, join_style='mitre').contains(gdf_merge.at[i, 'geometry']):
                logger.debug(f"Skip  catchment catch_id: {gdf_merge.at[i, 'catch_id']:>8}, "
                             f"since it is covered by catchment {catch_id}.")
                count_skip += 1
                continue
            index = gdf_keep[gdf_keep['catch_id'] == catch_id].index.values[0]  # index in gdf_keep
            logger.debug(
            ...
            geom_update = (geom_update_t.buffer(EPS, join_style='mitre').
            ...
            assert gdf_keep[gdf_keep['catch_id'] == catch_id]['geometry'].values[0] == geom_update, \
                f"Did not assign geometry value successfully. index: {index} catch_id: {catch_id}"
            assert gdf_keep[gdf_keep['catch_id'] == catch_id]['area'].values[0] == geom_update.area, \
                f"Did not assign area value successfully. index: {index} catch_id: {catch_id}"
            count_merge += 1
        elif gdf_merge.at[i, 'area'] <= lower_area:  # to abandon
            logger.debug(f"Skip tiny catch_id: {gdf_merge.at[i, 'catch_id']} with area: {gdf_merge.at[i, 'area']}.")
            count_skip += 1
    ...
    if count_standalone > 0:
        logger.info(f"Added {count_standalone} standalone catchments into catchment table.")
    ...
    if not gdf_keep_gap.empty:
        gdf_exist_gap = gdf_keep[
            (gdf_keep['catch_id'] >= GAP_OFFSET - GAP_REVISE) & (gdf_keep['catch_id'] < GAP_OFFSET)
            ].copy()
        if not gdf_exist_gap.empty:
            init_id = gdf_exist_gap['catch_id'].max() + 1
        else:
            init_id = GAP_OFFSET - GAP_REVISE
        gdf_keep_gap = gdf_keep_gap.sort_values(by='area', ascending=False).reset_index(drop=True)
        gdf_keep_gap['catch_id'] = gdf_keep_gap.index + init_id
        logger.debug(f"Processed {len(gdf_keep_gap)} gaps catch_id and save them as new catchments, "
                     f"catch_id: {gdf_keep_gap['catch_id'].to_list()}.")
        gdf_result = pd.concat([gdf_keep[gdf_keep['catch_id'] < GAP_OFFSET].copy(), gdf_keep_gap], ignore_index=True)
        gdf_result = gdf_result.sort_values(by='catch_id', ascending=True).reset_index(drop=True)
    else:
        gdf_result = gdf_keep.sort_values(by='catch_id', ascending=True).reset_index(drop=True)
        logger.debug(f"No new catchments form gaps area are found.")
    assert not (gdf_result['catch_id'] >= GAP_OFFSET).any(), f'catch_id should be smaller than {GAP_OFFSET}.'
    gc.collect()
    return gdf_result
...
def trim_single_catchment(gds: Union[gpd.GeoSeries, pd.Series]) -> tuple:
    ...
    geom_fil = (geom.buffer(-(CATCHMENT_RESOLUTION / 2 + EPS), join_style='mitre').
                buffer((CATCHMENT_RESOLUTION / 2 + EPS), join_style='mitre'))
    geom_fil = utils.filter_geometry(geom_fil,
                                     resolution=CATCHMENT_RESOLUTION,
    ...
    if geom_mask.area > CATCHMENT_RESOLUTION * CATCHMENT_RESOLUTION * 8:
        ...
        geom = utils.filter_geometry(geom,
                                     resolution=CATCHMENT_RESOLUTION,
                                     polygon_threshold=LOWER_AREA)
        ...
        gdf_result = gpd.GeoDataFrame(index=[0], crs='epsg:2193', geometry=[gds['geometry']])
...
def align_single_catchment(index: int, table: Type[tables.Ttable]) -> gpd.GeoDataFrame:
    """
    align input geometry with corresponding superior sea draining catchments.
    TODO: not bug free, need to be improved.
    """
    engine = utils.get_database(null_pool=True)
    gdf = tables.get_data_by_id(engine, table, index)
    ...
    buffer = (CATCHMENT_RESOLUTION * 3 + EPS if utils.get_min_width(geom) > CATCHMENT_RESOLUTION * 3 else 10)
    gdf_sdc = tables.get_within_catchment_by_geometry(engine, tables.SDCP, geom, -buffer)
    ...
    if no_superior or no_intersect:
        logger.debug(f"{table.__tablename__}, {index:>8}, no superior catchment: {no_superior}, or "
                     f"no intersect with superior catchment: {no_intersect}, nothing to do.")
        gdf_result = gpd.GeoDataFrame(index=[0], crs='epsg:2193', geometry=[geom])
        gdf_result['area'] = gdf_result['geometry'].area
        gdf_result['catch_id'] = index
        return gdf_result
    logger.debug(f"{table.__tablename__}, {index:>8}, "
    ...
    if isinstance(geom_dif, shapely.geometry.MultiPolygon):
        geom_dif = max(geom_dif.geoms, key=lambda x: x.area)  # select the biggest polygon
...
def align_catchments(list_id: list,
                     table: Type[tables.Ttable],
                     parallel: bool = True,
                     gpkg: bool = False) -> gpd.GeoDataFrame:
    """
    ...
    if parallel:
        with Pool(processes=multiprocessing.cpu_count()) as pool:
            gdf_result = pool.starmap(align_single_catchment, zip(list_id, repeat(table)))
            pool.close()
            pool.join()
...
def refine_catchments(table: Type[tables.Ttable],
                      trim: bool = False,
                      parallel: bool = True,
                      gpkg: bool = False) -> None:
    """
    ...
    """
    assert table.__tablename__ in GAP_TABLE_MAPPING.keys(), \
    ...
    gdf_gaps = find_gaps(gdf,
                         table=GAP_TABLE_MAPPING[table.__tablename__],
                         gpkg=gpkg)
    ...
    engine.dispose()
    gc.collect()
    logger.info(f"Finish refine catchments, get total {len(gdf_to_db)} catchments and "
                f"save to table {table.__tablename__}.")


# @utils.timeit  # 0:01:51.943599 for Order5P and Order4P, trimming only.
def refine_sub_catchments(trim: bool = True, align: bool = False, parallel: bool = True, gpkg: bool = False) -> None:
    """
    ...
    for table in list_table:
        ...
        engine.dispose()
        gc.collect()
        logger.info(f"Finish refining {table.__tablename__} catchments, get {len(gdf_to_db)} processed catchments.")
    logger.info(f"Finish refining all subordinated catchments data.")


# @utils.timeit  # 0:10:15.315367
def gen_coast_catchments(coast_distance: Union[int, float] = COAST_DISTANCE,
    ...
    geom_catchments = utils.filter_geometry(gdf_catchments['geometry'].copy(),
                                            resolution=CATCHMENT_RESOLUTION,
                                            polygon_threshold=lower_area)
    ...
    geom_coast = utils.filter_geometry(geom_coast,
                                       resolution=CATCHMENT_RESOLUTION,
                                       polygon_threshold=lower_area,
                                       hole_threshold=100 * 100)
    # cut into grid catchments
    list_fishnet = utils.fishnet(geom_coast, threshold=COAST_GRID)
    gdf_grid = gpd.GeoDataFrame(index=range(COAST_OFFSET, COAST_OFFSET + len(list_fishnet)),
                                crs='epsg:2193',
    ...
    gdf_to_db.to_postgis(tables.COAST.__tablename__, engine, index=True, if_exists="replace")
    ...
    gdf_to_db.to_postgis(tables.CATCHMENT.__tablename__, engine, index=True, if_exists="replace", chunksize=4096)
    engine.dispose()
    gc.collect()
    logger.info(f"Finish generating coast catchments table, "
                f"add {len(gdf_grid)} coast catchments in coast_catchment table, "
                f"catch_id range:\n{gdf_grid['catch_id'].min()} - {gdf_grid['catch_id'].max()}")
    logger.info(f"Finish generating all catchments table, total {len(gdf)} catchments in catchment table, "
...
def split_catchment(gds: Union[pd.Series, gpd.GeoSeries]) -> tuple:
    ...
    engine = utils.get_database(null_pool=True)
    gdf_order5 = find_subordinate(engine, tables.ORDER5P.__tablename__, gds['geometry'], buffer)
    gdf_order5 = gdf_order5[gdf_order5['area'] < UPPER_AREA] if not gdf_order5.empty else gdf_order5
    ...
    if gdf_order5.empty:
        logger.debug(f"Splitting {gds['catch_id']:>8}, step {step}: order5 fail,\torder 4 trying...")
        geom_dif = utils.filter_geometry(gds['geometry'],
                                         resolution=CATCHMENT_RESOLUTION,
                                         polygon_threshold=LOWER_AREA)
        rest_area = gds['area']
    else:
        logger.debug(f"Splitting {gds['catch_id']:>8}, "
                     f"step {step}: order5 success:\t{gdf_order5['catch_id'].to_list()}")
        gdf_concat = pd.concat([gdf_concat, gdf_order5], ignore_index=True)
        geom = utils.filter_geometry(gdf_order5['geometry'].copy(),
                                     resolution=CATCHMENT_RESOLUTION,
                                     polygon_threshold=LOWER_AREA)
        geom_dif = gds['geometry'].difference(geom)
        geom_dif = utils.filter_geometry(geom_dif,
                                         resolution=CATCHMENT_RESOLUTION,
                                         polygon_threshold=LOWER_AREA)
    ...
    if rest_area > UPPER_AREA:
        gdf_order4 = find_subordinate(engine, tables.ORDER4P.__tablename__, geom_dif, buffer)
        gdf_order4 = gdf_order4[gdf_order4['area'] < UPPER_AREA] if not gdf_order4.empty else gdf_order4
        if gdf_order4.empty:
            logger.warning(f"Splitting {gds['catch_id']:>8}, "
            ...
            geom = utils.filter_geometry(gdf_order4['geometry'].copy(),
                                         resolution=CATCHMENT_RESOLUTION,
                                         polygon_threshold=LOWER_AREA)
        ...
        if rest_area > UPPER_AREA:
            ...
            gdf_rest = gpd.GeoDataFrame(index=range(len(list_katana)),
                                        crs='epsg:2193',
                                        geometry=list_katana)
            ...
            gdf_rest['catch_id'] = gds['catch_id'] * SPLIT_SHIFT + SPLIT_OFFSET + gdf_rest.index.values
    ...
    gdf_rest = gdf_rest[['catch_id', 'area', 'geometry']]
    gdf_concat = pd.concat([gdf_concat, gdf_rest], ignore_index=True)
    logger.debug(f"Splitting {gds['catch_id']:>8}, step {step}: residue done:\t{gdf_rest['catch_id'].to_list()}")
    gdf_split = gdf_concat.copy()
    gdf_split['super_id'] = gds['catch_id']
    engine.dispose()
    gc.collect()
...
def gen_catchment_table(parallel: bool = True,
                        gpkg: bool = False) -> None:
    """
    ...
    gdf_to_db = tables.get_catchment_by_area_range(engine,
                                                   tables.SDCP,
    ...
    gdf_to_split = tables.get_catchment_by_area_range(engine, tables.SDCP,
                                                      upper_area=None,
    ...
    logger.info(f"Splitting catchments with area > {UPPER_AREA}, total {len(gdf_to_split)}")
    logger.debug(f"Splitting catchments catch_id:\n{gdf_to_split['catch_id'].to_list()}")
    if parallel:
    ...
    gdf_to_db.to_postgis(tables.CATCHTEMP.__tablename__, engine, index=True, if_exists="replace", chunksize=4096)
    ...
    gdf_split.to_postgis(tables.SDCS.__tablename__, engine, index=True, if_exists="replace")
    logger.info(f"Save split catchments superior-subordinate mapping to {tables.SDCS.__tablename__} table.")
    engine.dispose()
    gc.collect()
...
def run(gpkg: bool = False) -> None:
    ...
    deduplicate_table([tables.SDC, tables.ORDER5, tables.ORDER4],
